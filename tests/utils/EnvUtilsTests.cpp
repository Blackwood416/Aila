...
void default_kv_cache_honors_cli_global_override(TestResults& results) {
    clear_kv_env();
    set_env_var("AILA_KV_QUANT", "0");
    aila::env::g_kv_quant_override = true;
    ...
    Context ctx;
    KVCache cache;
    cache.init(ctx, config, 1);
    AILA_EXPECT_TRUE(results, cache.quantized());
}
...
int main() {
    ...
    default_kv_cache_honors_cli_global_override(results);
    clear_kv_env();
