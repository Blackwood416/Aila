...
void erase_with_prefix_removes_only_matching_weights(TestResults& results) {
    ...
    weights.put("speaker_encoder.blocks.0.conv.weight", Tensor{});
    weights.put("speaker_encoder.fc.bias", Tensor{});
    weights.put("talker.model.norm.weight", Tensor{});

    const ModelWeightsEraseStats stats = weights.erase_with_prefix("speaker_encoder.");
    ...
    AILA_EXPECT_TRUE(results, !weights.has("speaker_encoder.blocks.0.conv.weight"));
    AILA_EXPECT_TRUE(results, !weights.has("speaker_encoder.fc.bias"));
