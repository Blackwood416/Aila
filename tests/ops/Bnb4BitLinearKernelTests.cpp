...
int run_packed_nf4_gemm_layout_test(int M) {
    constexpr int N = 128;
    constexpr int K = 128;
    ...
    double expected_at_max_diff = 0.0;
    double actual_at_max_diff = 0.0;
    ...
    for (int m = 0; m < M; ++m) {
        for (int n = 0; n < N; ++n) {
            ...
            const double tolerance = std::max(0.06, 0.005 * std::abs(expected));
            if (diff > tolerance) {
                exceeds_tolerance = true;
            }
            if (diff > max_abs_diff) {
                max_abs_diff = diff;
                expected_at_max_diff = expected;
                actual_at_max_diff = actual;
            }
        }
    }

    if (exceeds_tolerance) {
        std::cerr << "packed_nf4_gemm_bf16 M=" << M
                  << " max_abs_diff=" << max_abs_diff
                  << " expected=" << expected_at_max_diff
                  << " actual=" << actual_at_max_diff
                  << " max_relative_diff=" << max_relative_diff
                  << " exceeds tolerance\n";
        return 1;
    }

    std::cout << "packed_nf4_gemm_bf16 M=" << M
              << " max_abs_diff=" << max_abs_diff
              << " max_relative_diff=" << max_relative_diff << "\n";
    return 0;
}
...
int main() {
    const int failed = run_packed_nf4_gemm_layout_test(3) |
                       run_packed_nf4_gemm_layout_test(96);
    if (failed != 0) {
